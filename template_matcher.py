...
class TemplateMatcher:
    ...
    def match_template_with_confidence(self, screen, templates):
        ...
        for template_path in templates:
            ...
            confidences = [result[pt[1], pt[0]] for pt in zip(*locations[::-1])]
    ...
    def non_max_suppression(self, boxes):
        ...
        indices = cv2.dnn.NMSBoxes(
            bboxes=[[int(x), int(y), int(w), int(h)] for x, y, w, h in boxes[:, :4]],
    ...
    def draw_boxes_with_confidence(self, screen, boxes, label):
        ...
        for (x, y, w, h, conf) in boxes:
            cv2.rectangle(screen, (int(x), int(y)), (int(x + w), int(y + h)), (0, 255, 0), 2)
            text = f"{label}: {conf:.2f}"
            cv2.putText(screen, text, (int(x), int(y - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        return screen

    def refresh_and_display(self):
        """Capture the screen, match templates, and display results."""
        screen = self.capture_window_image()
        chat_boxes = self.match_template_with_confidence(screen, self.chat_templates)
        private_chat_boxes = self.match_template_with_confidence(screen, self.private_chat_templates)
        private_chat_content_boxes = self.match_template_with_confidence(screen, self.private_chat_content_templates)

        chat_boxes = self.non_max_suppression(chat_boxes)
        private_chat_boxes = self.non_max_suppression(private_chat_boxes)
        private_chat_content_boxes = self.non_max_suppression(private_chat_content_boxes)

        screen_with_boxes = self.draw_boxes_with_confidence(screen, chat_boxes, "Chat Box")
        screen_with_boxes = self.draw_boxes_with_confidence(screen_with_boxes, private_chat_boxes, "Private Chat Box")
        screen_with_boxes = self.draw_boxes_with_confidence(screen_with_boxes, private_chat_content_boxes, "Private Chat Content")
    ...
    def run(self):
        ...
        while True:
            ...
            if key == 27:  # ESC key to exit
                break
            elif key == 32:  # SPACE key to refresh
                self.refresh_and_display()
